...
def compute_zobrist_hash(gs):
    ...
    if gs.whiteToMove:
        h ^= random.getrandbits(64)  # hoặc tạo hằng số cố định cho whiteToMove
    # Optional: xor thêm cho castling rights, en passant (có thể mở rộng nếu cần)
    return h
...
def findMoveNegaMaxAlphaBeta(gs, validMoves, depth, alpha, beta, turnMultiplier):
    ...
    # Tính hash trạng thái hiện tại
    zobrist_hash = compute_zobrist_hash(gs.board)

    # Kiểm tra xem đã tính trạng thái này chưa ở độ sâu >= hiện tại
    ...
    for move in validMoves:
        ...
        if alpha >= beta:  # if we find new max is greater than minimum so far in a branch then we stop iterating in that branch as we found a worse move in that branch
            break
    transposition_table[zobrist_hash] = {'score': maxScore, 'depth': depth}
    return maxScore
